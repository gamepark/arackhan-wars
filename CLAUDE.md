...
rules/src/                    # Server-side game logic
  ...
  │   └── cards/              # Card data by faction (Blight, Grey Order, Nakka, Whitelands)
  ├── rules/
  │   ├── RuleId.ts           # Game phases enum
  │   └── *Rule.ts            # Rule implementations
  ├── ArackhanWarsRules.ts    # Main rules class
  ├── ArackhanWarsSetup.ts    # Initial game setup
  └── ArackhanWarsOptions.ts  # Game configuration

app/src/                      # Client-side React UI
  ├── material/Material.tsx   # Visual descriptions (sizes, images)
  ├── locators/Locators.ts    # Positioning on screen
  ├── headers/Headers.tsx     # In-game text display
  ├── deckbuilding/           # Deck building UI
  ├── tutorial/               # Tutorial mode
  ├── history/                # Move history
  └── images/                 # Game assets (card images by locale)
```
...
- `materialI18n` prop on `GameProvider` — localized material descriptions (card images per language)
