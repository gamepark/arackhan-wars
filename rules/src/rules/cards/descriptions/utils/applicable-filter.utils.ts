...
export type ApplicableFilter = (_source: Material, _target: Material, cardDescription: GetCardDescription) => boolean
...
export const family = (family: string) => (_source: Material, target: Material, cardDescriptionHelper: GetCardDescription) => {
  return cardDescriptionHelper.get(target.getIndex()).family === family
}
...
export const or = (...filters: ApplicableFilter[]) => (source: Material, target: Material, cardDescriptionHelper: GetCardDescription) => filters.some(filter => filter(source, target, cardDescriptionHelper))
