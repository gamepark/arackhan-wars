...
import { LocationRulesProps, MaterialComponent, pointerCursorCss, usePlay, usePlayerName, useRules } from '@gamepark/react-game'
...
export const PlayerDiscardRules = ({ location }: LocationRulesProps) => {
  ...
  const play = usePlay()
  return <>
    <h2>{t('rules.discard.title', { player })}</h2>
    <p>{t('rules.discard.content', { number: cards?.length, player })}</p>
    <ol css={grid}>
